...
class UNO(nn.Module):
    def __init__(
        ...
        self.fc0 = nn.Conv2d(
            nin + 2 if conv_type == "spectral" else nin, width, 1
        )
        ...
        if isinstance(res, int):
            ...
            raise ValueError(
                "res must be either an integer or a tuple of two integers."
            )
                ...
                layers.append(
                    nn.ModuleList(
                        [
                            ...
                            (
                                FlashAttnBlock(dim_in)
                                if min(cur_height, cur_width) <= attn_res
                                else Identity()  
                            ),
                        ]
                    )
        ...
        self.fc1 = nn.Conv2d(width, int((width + nout) / 2), 1)
        self.fc2 = nn.Conv2d(int((width + nout) / 2), nout, 1)
...
class FlashAttnBlock(nn.Module):
    ...
    def forward(self, x, t=None, z=None):
        ...
        )  # Note that the c here refers to dim head
...
#     def __init__(self, dim, min_heads=4, dim_per_head=36):
#         super().__init__()
#         self.num_heads = num_heads = max( (dim // dim_per_head) + 1, min_heads)
#         self.norm = nn.LayerNorm(dim)
#         self.qkv = nn.Linear(dim, num_heads * dim_per_head * 3)
#         self.attn_linear = nn.Linear(num_heads * dim_per_head, dim)

#         # Using nn.MultiheadAttention
#         self.attn = nn.MultiheadAttention(
#             embed_dim=(num_heads * dim_per_head), num_heads=num_heads
#         )
...
#         # Split qkv and pass them to MultiheadAttention
#         qkv = self.qkv(h)  # (b, l, 3 * num_heads * dim_per_head)

#         q, k, v = qkv.chunk(3, dim=-1)  # Split into Q, K, V

#         # MultiheadAttention expects input as (seq_len, batch, num_heads * dim_per_head), so we transpose
...
#         attn_output = attn_output.transpose(0, 1)  # (b, l, num_heads * dim_per_head)
#         # Linear layer to map back to original dimension
#         h = self.attn_linear(attn_output) # (b, l, dim)
...
class SpectralConv2d(nn.Module):
    def __init__(
        ...
        self.modes1 = (
            modes1  # Number of Fourier modes to multiply
        )
        self.modes2 = modes2
