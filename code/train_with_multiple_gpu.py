...
def train_on_batch(
    ...
    *,
    ...
    if global_step % H.train.plot_graph_steps == 0:
        ...
        if rank == 0:
            print(
                f"Step: {global_step}, Loss: {mean_loss / norm:.5f}, Step Time: {mean_step_time / norm:.5f}, Skip Rate: {skip / norm:.5f}, Gradient Norm: {mean_total_norm / norm:.5f}"
            )
            wandb_dict |= {
...
def train(
    ...
    # Iterate through each DataLoader sequentially
    while True:
        for dataloader in train_loaders:
            for batch in dataloader:
                global_step += 1
                mean_loss, mean_step_time, skip, mean_total_norm = train_on_batch(
                    batch,
                    H=H,
                    ...
                    q_sample_ratio=H.mc_integral.q_sample_ratio
                )
